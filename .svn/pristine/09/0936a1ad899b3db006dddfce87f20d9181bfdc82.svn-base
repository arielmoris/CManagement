...
function View_Chart(xml,chart_kind,width,height){
//	alert(xml);
//	alert(chart_kind);
	xml_arr = xml.split("&&");
	for(i=0;i<xml_arr.length;i++){
		document.getElementById('chartdiv').innerHTML +="<div id='chartdiv"+i+"'></div>";
	}
	 for(i=0;i<xml_arr.length;i++){
		 var chart = new FusionCharts("../dashboards/"+chart_kind+".swf", "ChartId"+i, width, height, "0", "0", "FFFFFF", "noscale");
		 
		 chart.setDataXML(xml_arr[i]);
