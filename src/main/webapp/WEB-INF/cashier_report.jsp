...
<!-- saved from url=(0045)cashier_report.jsp -->
  ...
  <div class="system">Management System</div>
      ...
      <div class="tabs_table">
        <table summary="" cellpadding="0" cellspacing="0" border="0">
          <tbody><tr>
            <td class="tabs0">&nbsp;&nbsp;</td><td class="tabs1">Cashier&nbsp;Report </td><td class="tabs4">&nbsp;</td>
          </tr>
        </tbody></table>
      </div>
...
//	alert(count);
	if(count == 1){
		document.getElementById('use_ho_tz').style.display='block';
	}
	else{
		if(document.getElementById('use_ho_tz')){
			document.getElementById('use_ho_tz').style.display='none';
			document.getElementById('use_ho_timezone').checked=false;
		}
		 
	}
//alert(document.getElementById('use_ho_timezone').checked);
}

function check_select(select)
{   
	if(select.value=='detailed')
		disable=false;
	else 
	{
	...
	if(document.getElementById('currency'))
	{
		...
		else disable_field('currency',false);
	}
...
function SubmitReport(summaryCurrency)
{
	var rowsToShow = '500';
    var txt='';
	var SessionLoginId=168025;
	select_list_arr='';
	from_date_arr=document.getElementById('from_date').value.split('-');
	to_date_arr=document.getElementById('to_date').value.split('-');	
	...
	fromDate.setFullYear(from_date_arr[0], from_date_arr[1] - 1, from_date_arr[2]);
	toDate.setFullYear(to_date_arr[0], to_date_arr[1] - 1, to_date_arr[2]);
	
	var compareDate = (toDate - fromDate) / (24 * 60 * 60 * 1000) ;
	
	//debugger;
	
	if(select_summary_level == 'detailed' && compareDate > 62 /*(from_date_arr[0] != to_date_arr[0] || from_date_arr[1] != to_date_arr[1])*/) //should be same month and year
	{
	...
	var time_zone_select='own';
	if(document.getElementById('time_zone_select') && (document.getElementById('time_zone_select').disabled==false))
		time_zone_select=document.getElementById('time_zone_select').value;

	//alert(select_list_arr);
	//alert(select_summary_level);
	if((select_summary_level=='detailed') && document.getElementById('from_time_hour') && document.getElementById('to_time_hour'))
	{
	...
	var params = '' + 
        'CashierReportSummaryLevel-' + select_summary_level + '$$' + 
        'EntityList_XML-' + select_list_arr + '$$' + 
        'CurrencyId-' + currency + '$$' + 
        'SummaryCurrencyId-' + summaryCurrency + '$$' + 
        'TimeZoneType-' + time_zone_select + '$$' + 
        'Top-' + rowsToShow;
...
if(document.getElementById("summary_level"))
  ...
  <td class="left" nowrap="nowrap" style = "padding-left:15px;">
      <div class="welcome">
					...
					<script type="text/javascript">				
						setTimeout('sndReqBalance(\'Test168025\',\'cashier_report\',300)',300*1000);
					</script>
					<span id="balance">
					<div class="balance_casino">
						<b>Credit Balance 100,000.00  KRW</b>
					</div>
				</span>Welcome, <strong>Test1 AAagent</strong><br>Agent<br><em><font size="1px"><span id="gmt_time">1/9/2015 17:42</span><br>Time zone: GMT +9:00 (Daylight: Off) </font></em><br>
