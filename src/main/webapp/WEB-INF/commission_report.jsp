  ...
  <div class="system">Management System</div>
      ...
      <div class="tabs_table">
        <table summary="" cellpadding="0" cellspacing="0" border="0">
          <tbody><tr>
            <td class="tabs0">&nbsp;&nbsp;</td><td class="tabs1">Commission&nbsp;Report </td><td class="tabs4">&nbsp;</td>
          </tr>
        </tbody></table>
      </div>
	...
	<div class="row">
 	<div class='col-sm-6'>
		...
		<script type="text/javascript">
			try {
				document.getElementById("report_output_td").innerHTML = '<input type="hidden" name="report_output" id="report_output" value="1">';
			}
			catch(err) {}
		</script>
...
<script type="text/javascript">
	...
	try {
		document.getElementById('bala1_td').innerHTML='';
	}
	catch(err) {}
	...
	function SubmitReport(summaryCurrency)
	{
		...
		if(CheckDate(start_date, end_date)) {
			...
			var ReportOutput = '';
			var IncludeTestMembers = 0;
			if (document.getElementById('include_test_members')) {
				if (document.getElementById('include_test_members').checked == true ) IncludeTestMembers = 1;
			}
			/*
				var PokerTournamentTypeId = '';
				var PokerTournamentHostsTypeId = '';
				var SearchType = '';
			*/
			...
			if(document.getElementById('select_list')) {
				for(i = 1; i < document.getElementById('select_list').length; i++) {	
					if(document.getElementById('select_list')[i].selected == true)
						tmp_arr += document.getElementById('select_list')[i].value + ',';
				}
				select_list_arr = tmp_arr.substr(0, tmp_arr.length-1);
				tmp_arr = '';
			}
		
			var EntityTypeId = '4';
			var ReportOutput = document.getElementById('report_output').value;
			
			/*
				var PokerTournamentTypeId = document.getElementById('poker_type').value;
				var PokerTournamentHostsTypeId = document.getElementById('host_type').value;
				if (document.getElementById('IsOnlyGuaranteedPrizeAmount').checked == true) var IsOnlyGuaranteedPrizeAmount = 1;
				else var IsOnlyGuaranteedPrizeAmount = '';
			*/
			...
			var params = '' + 
				...
				'IncludeTestMembers-' + IncludeTestMembers + '$$' + 
				...
				'PokerTournamentTypeId-' + PokerTournamentTypeId + '$$' + 
				'PokerTournamentHostTypeId-' + PokerTournamentHostsTypeId + '$$' + 
				'IsOnlyGuaranteedPrizeAmount-' + IsOnlyGuaranteedPrizeAmount;
				*/
				sndReqGenericReport('get_generic_report.php', 'CommissionReport', start_date, end_date, params, txt, '', 0, 0, session);
		}
	}
  ...
  <td class="left" nowrap="nowrap" style="padding-left:15px;">
      <div class="welcome">
					...
					<script type="text/javascript">				
						//setTimeout('sndReqBalance(\'Test168025\',\'commission_report\',300)',300*1000);
					</script>
					<span id="balance">
					<div class="balance_casino">
						<b>Credit Balance 100,000.00  KRW</b>
					</div>
				</span>Welcome, <strong>Test1 AAagent</strong><br>Agent<br><em><font size="1px"><span id="gmt_time">1/9/2015 14:10</span><br>Time zone: GMT +9:00 (Daylight: Off) </font></em><br>
