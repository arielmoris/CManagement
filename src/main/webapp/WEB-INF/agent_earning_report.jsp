...
<!-- saved from url=(0051)https://www.totalegame.net/agent_earning_report.php -->
...
<style type="text/css">
.scw{padding:1px;vertical-align:middle;}iframe.scw     {position:absolute;z-index:1;top:0px;left:0px;visibility:hidden;width:1px;height:1px;}table.scw      {padding:0px;visibility:hidden;position:absolute;cursor:default;width:160px;top:0px;left:0px;z-index:2;text-align:center;}</style>
...
<table class="top" summary="" width="760" cellpadding="0" cellspacing="0" border="0">
  <tbody><tr valign="top">
	...
	<script type="text/javascript">
        ...
        function CheckSelectList(select) {
        	var count = 0
        ...
        function check_select(select) {
            if (select.value == 'Detailed' || select.value == 'ByMemberByDay') {
                disable = false;
            }
            else {
                set_field_val('from_time_hour', '00');
                set_field_val('from_time_min', '00');
                set_field_val('to_time_hour', '23');
                set_field_val('to_time_min', '59');
                disable = true;
            }
            ...
            if (document.getElementById('currency')) {
                if (select.value == 'Daily') {
                    ...
                    disable_field('currency', true);
                }
                else {
                    disable_field('currency', false);
                }
            }
	
            disable_field('from_time_hour', disable);
            disable_field('from_time_min', disable);
            disable_field('to_time_hour', disable);
            disable_field('to_time_min', disable);
        }

        function disable_field(field, val) {
            if (document.getElementById(field)) {
                document.getElementById(field).disabled = val;
            }
        }
        ...
        function SubmitReport(summaryCurrency) {
            var select_list_arr = '3927689';
            var customer_list_arr = '';
            var currency = 10;
        	var time_zone_type = 'Own';
	        var include_test='';
            ...
            if (document.getElementById('time_zone_type')) {
                time_zone_type = document.getElementById('time_zone_type').value;
            }

        	if ((select_summary_level=='Detailed' || select_summary_level == 'ByMemberByDay')
                && document.getElementById('from_time_hour')
                && document.getElementById('to_time_hour')) {

                from_datetime = document.getElementById('from_date').value + ' ' + document.getElementById('from_time_hour').value + ':' + document.getElementById('from_time_min').value;
                to_datetime = document.getElementById('to_date').value + ' ' + document.getElementById('to_time_hour').value + ':' + document.getElementById('to_time_min').value;
            }
            else {
                from_datetime = document.getElementById('from_date').value + ' 00:00';
                to_datetime = document.getElementById('to_date').value + ' 23:59';
            }
            ...
            if (CheckDate(from_datetime, to_datetime)) {
            

                
                var useHOtz = false;
                var txt = '';
                var summaryLevel = document.getElementById('summary_level').value;
                ...
                var params = '' +
                    'CustomerTypeId-' + '$$' +
                    'CustomerId-' + select_list_arr + '$$' +
                    'CurrencyId-' + currency + '$$' +
                    'SummaryCurrencyId-' + summaryCurrency + '$$' +
                    'CurrencyRateTypeId-' + currencyRateTypeId + '$$' +
                    'SummaryLevel-' + summaryLevel + '$$' +
                    'TimeZoneType-' + time_zone_type + '$$' +
                    'IncludeTestMembers-' + include_test;
                ...
                /*sndReqEarningReport(
                    'get_agent_earning_report_by_day.php',
                    from_datetime,
                    to_datetime,
                    select_list_arr,
                    select_summary_level,
...
if(document.getElementById("select_list"))
	CheckSelectList(document.getElementById("select_list"));
</script>
    ...
    <td class="left" nowrap="nowrap">
      <div class="welcome">
					...
					<script type="text/javascript">				
						setTimeout('sndReqBalance(\'Test168025\',\'agent_earning_report\',300)',300*1000);
					</script>
					<span id="balance">
					<div class="balance_casino">
						<b>Credit Balance 100,000.00  KRW</b>
					</div>
				</span>Welcome, <strong>Test1 AAagent</strong><br>Agent<br><em><font size="1px"><span id="gmt_time">1/23/2015 19:33</span><br>Time zone: GMT +9:00 (Daylight: Off) </font></em><br>
	...
	<input type="button" class="logout" value="Logout" onclick="location.href='logout.php'" style="cursor:pointer;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" class="logout" style="width:120px;" value="Change Password"></form>	
	<script type="text/javascript">
